...
const TableTab: React.FC<any> = ({ sortedTable, powerRankedTeams, setSelectedTeamProfile, onShareTable }: any) => {
  const minGa = sortedTable.filter((t: any) => (t.played || 0) > 0).reduce((min: number, t: any) => ((t.ga || 0) < min ? (t.ga || 0) : min), Infinity);
            ...
            <div className="overflow-x-auto custom-scrollbar">
              <table className="w-full text-right border-collapse">
                ...
                <tbody className="divide-y divide-zinc-800/50 text-sm md:text-base">
                  {sortedTable.map((t: any, i: number) => {
                    ...
                    const pTeam = powerRankedTeams.find((pt: any) => pt.id === t.id);
