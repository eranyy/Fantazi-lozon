...
const applySubstitutionsToLineup = (team: Team, currentRound: number): Player[] => {
    ...
    const roundSubs = safeArray(team.transfers).filter((t: any) =>
        t && t.type === 'HALFTIME_SUB' && Number(t.round) === Number(currentRound) && (t.status || '').toUpperCase() !== 'CANCELLED'
    );
...
const getTeamLiveEvents = (team: Team, currentRound: number): { goals: number, yellows: number, reds: number } => {
    ...
    const subbedOutPlayers = safeArray(team.transfers)
        .filter((t: any) => t && t.type === 'HALFTIME_SUB' && Number(t.round) === Number(currentRound) && (t.status || '').toUpperCase() !== 'CANCELLED')
        .map((sub: any) => {
...
const askGeminiFantasyAI = async (userPrompt: string, senderPhone: string = '', chatId: string = ''): Promise<string> => {
    ...
    try {
        ...
        const calculateOptimalLineupServer = (squad: any[]) => {
        ...
        if (isStructuredAiQuery && !isControlCmd) {
            ...
            if (p.includes('טבלה') || p.includes('מי מוביל') || p.includes('מקום ראשון') || p.includes('מי בראש')) {
                ...
                if (last) msg += `📉 *בתחתית הבוערת:* *${last?.teamName}* (${last?.manager}) עם ${last?.totalPoints} נק' בלבד.\n\n`;
        ...
        if (isApprove || isReject) {
            ...
            for (const d of usersSnap.docs) {
                ...
                if (updated) {
                    ...
                    const updatedRLineup = (currentRData.lineup || lineup).map((pl: any) => {
                        if (norm(pl.name).includes(norm(matchedPlayerName)) || norm(matchedPlayerName).includes(norm(pl.name))) {
                            const currentStats = pl.stats || {};
                            return {
                                ...pl,
        ...
        if (eventKeywords.some(kw => p.includes(kw))) {
            try {
                ...
                const smartNorm = (s: string) => String(s || '').toLowerCase().replace(/[^\u0590-\u05FFa-zA-Z0-9]/g, '').replace(/יי/g, 'י').replace(/וו/g, 'ו');
                ...
                if (matchedPlayer && matchedDocId && matchedUserData) {
                    ...
                    if (isSixtySingle || isSubOff || isSubOn || isNoPlay) {
                        ...
                        if (isCancel && !isGoal && !isAssist && !isYellow && !isRed && !isConceded && !isPenWon && !isPenConceded && !isPenSaved && !isPenMissed) {
                                ...
                                if (pl.id === matchedPlayer.id || pl.name === matchedPlayer.name || plNorm.includes(matchedNorm) || matchedNorm.includes(plNorm)) {
                                    ...
                                    return {
                                        ...
                                        stats: {
                                            ...
                                            penaltyWon: isCancel && isPenWon ? Math.max(0, (currentStats.penaltyWon || 0) - 1) : isPenWon ? (currentStats.penaltyWon || 0) + 1 : (currentStats.penaltyWon || 0),
                                            penaltyConceded: isCancel && isPenConceded ? Math.max(0, (currentStats.penaltyConceded || 0) - 1) : isPenConceded ? (currentStats.penaltyConceded || 0) + 1 : (currentStats.penaltyConceded || 0),
                        ...
                        const managerNames = [u.manager, u.assistantName].filter(Boolean).join(' & ');
                        const arenaStatusStr = isInLineup ? `⚡ *השחקן בהרכב הפותח (${ptsAdd > 0 ? '+' : ''}${ptsAdd} נק') והניקוד בזירה באפליקציה עודכן בלייב!*` : '🪑 (השחקן נמצא בספסל המחליפים)';
                        const actionDescription = isCancel ? (isGoal ? 'בוטל לו השער!' : isYellow ? 'בוטל לו הכרטיס הצהוב!' : isRed ? 'בוטל לו הכרטיס האדום!' : 'בוטל האירוע!') : isGoal ? 'כבש גול במציאות!' : isAssist ? 'רשם בישול!' : isYellow ? 'ספג צהוב במציאות!' : isRed ? 'ספג אדום במציאות!' : isPenWon ? 'סחט פנדל במציאות! (+2 נק\')' : isPenConceded ? 'גרם לפנדל במציאות! (-2 נק\')' : isPenSaved ? 'עצר פנדל במציאות! (+3 נק\')' : isPenMissed ? 'החמיץ פנדל במציאות! (-3 נק\')' : isOwnGoal ? 'כבש שער עצמי!' : isConceded ? 'ספג שער במציאות!' : 'רשם אירוע ברשת!';
...
${chatHistoryContext ? `${chatHistoryContext}\n` : ''}`;
        ...
        if (p.includes('משחק') || p.includes('מחר') || p.includes('שבת') || p.includes('מתי')) {
            try {
                ...
                if (realSnap.exists) {
                    ...
                    const roundMatches = matches.filter((m: any) => m.round === curRound || !m.round).slice(0, 5);
                    if (roundMatches.length > 0) {
                        const listStr = roundMatches.map((m: any, i: number) => `${i + 1}. 🏟️ *${m.homeTeam}* 🆚 *${m.awayTeam}*\n   📅 בשעה *${m.time || '20:00'}* (${m.date || ''})`).join('\n\n');
            ...
            try {
                ...
                if (predSnap.exists) {
                    ...
                    if (standings.length > 0) {
                        const lines = standings.map((s: any, idx: number) => `${idx + 1}. *${s.name}* | ${s.points} נק' (${s.hits || s.correct || 0} ניחושים נכונים)`).join('\n');
        ...
        if (p.includes('טיפ') || p.includes('הרכב') || p.includes('להרכיב') || p.includes('סגל') || p.includes('מי לדעתך')) {
            try {
                ...
                if (squad.length > 0) {
                    const gk = squad.find((pl: any) => pl.position === 'GK' || pl.pos === 'GK');
                    const topPlayers = squad.filter((pl: any) => (pl.position !== 'GK' && pl.pos !== 'GK')).slice(0, 3);
                    const names = [gk, ...topPlayers].filter(Boolean).map((pl: any) => `• *${pl.name}* (${pl.realTeam || pl.team || ''})`).join('\n');
            ...
            try {
                ...
                if (body?.typeWebhook === 'incomingMessageReceived' || body?.messageData) {
                    ...
                    if (messageText) {
                        ...
                        const sendRes = await axios.post(`${greenHost}/waInstance${greenId}/sendMessage/${greenToken}`, {
                            chatId: chatId,
                            message: aiReply
                        });
...
const autoFinalizePredictorForRound = async (round: number) => {
    try {
        ...
        const roundFixture = roundsData.find((r: any) => r.round === round);
        if (!roundFixture || !roundFixture.isPlayed) return;

        const actualWinners: Record<string, string> = {};
        (roundFixture.matches || []).forEach((m: any) => {
            const hs = Number(m.hs || 0);
            const as = Number(m.as || 0);
            const key = `${m.h}_${m.a}`;
            if (hs > as) actualWinners[key] = m.h;
            else if (as > hs) actualWinners[key] = m.a;
            else actualWinners[key] = 'DRAW';
        });
        ...
        const existingStandings: any[] = standingsDoc.exists ? (standingsDoc.data()?.standings || []) : [];
        const standingsMap = new Map(existingStandings.map(s => [s.name, s]));

        Object.entries(votesData).forEach(([managerName, managerVotes]: [string, any]) => {
            let roundHits = 0;
            Object.entries(managerVotes || {}).forEach(([matchKey, pred]: [string, any]) => {
    ...
    try {
        ...
        try {
            ...
            if (realSnap.exists) {
                ...
                const nextRealMatches = realMatches.filter((m: any) => m.round === nextRound);
        ...
        try {
            ...
            if (!postsSnap.empty) {
                ...
                posts.sort((a: any, b: any) => new Date(b.timestamp || 0).getTime() - new Date(a.timestamp || 0).getTime());
        ...
        try {
            ...
            if (standings.length > 0) {
                const top3 = standings.slice(0, 3).map((s: any, i: number) => `${['🥇', '🥈', '🥉'][i]} *${s.name}* (${s.points || 0} נק')`).join(' | ');
        ...
        try {
            const playerMap: Record<string, { name: string, team: string, fantasyTeamName: string, points: number }> = {};
        ...
        if (imageUrl) {
            try {
                await axios.post(`${greenHost}/waInstance${greenId}/sendFileByUrl/${greenToken}`, {
                    chatId: groupChatId,
                    urlFile: imageUrl,
...
const runFridayPreRoundReminder = async (force: boolean = false) => {
    ...
    const teamsMap: Record<string, string> = { ...canonicalTeamNames };
    const missingTeams: any[] = [];
    ...
    const fantasyMatchesStr = (fantasyRound?.matches || []).map((m: any) => {
...
const runPredictorChaserLogic = async (): Promise<any> => {
    ...
    const realMatches = realSnap.data()?.matches || [];
    const upcoming = realMatches.filter((m: any) => (m.round === currentRound || !m.round) && !String(m.status || '').includes('הסתיים'));
    if (upcoming.length === 0) return { success: false, reason: 'No upcoming real matches' };
    ...
    for (const v of missingVoters) {
        try {
            await sendPushNotificationHelper(
                `🔮 אל תשכח להצביע בסקר הנביאים!`,
                `סקר הנביאים למחזור ${currentRound} ננעל בעוד ${timeStr}! כנס לווצאפ להצביע.`,
                v.teamId
            );
        } catch (pushErr) {
...
export const triggerPredictorChaser = onRequest({ region: 'us-west1', cors: true }, async (req, res) => {
    try {
...
export const scheduledFridayReminder = onSchedule({ schedule: '0 12 * * 5', timeZone: 'Asia/Jerusalem' }, async () => {
    try {
...
export const triggerFridayReminder = onRequest({ region: 'us-west1', cors: true }, async (req, res) => {
    try {
...
const runSheetSyncLogic = async () => {
    ...
    if (!csvData) {
        ...
        try {
            ...
            try {
                ...
                console.error('[runSheetSyncLogic] All sheet URLs failed:', (err as any)?.message);
            }
        }
    }
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (cols[0].includes('=') || cols[0].includes('תצוגה') || cols[0].includes('בחר') || cols[0].includes('ארכיון') || cols[0].includes('═')) continue;
        if (!cols[0].includes('מחזור') && !cols[0].includes('גמר') && !cols[0].includes('חצי') && !cols[0].includes('רבע')) continue;
...
const runLiveScraperLogic = async () => {
    ...
    try {
        ...
        const allTrackedPlayers: Array<{ name: string; realTeam: string; isDrafted: boolean }> = [];
        ...
        const checkAndCreatePending = async (plName: string, realTeam: string, eventType: string, source: string, desc: string) => {
            ...
            if (existingSnap.empty) {
                ...
                if (scraperSettings?.sendWhatsappPolls !== false) {
                    try {
                        ...
                        const pollRes = await axios.post(`${greenHost}/waInstance${greenId}/sendPoll/${greenToken}`, {
                            chatId: groupChatId,
        ...
        const extractLiveTickerSnippet = ($: any, selectors: string): string => {
...
export const updateLivePlayerPoints = onCall({ region: 'us-west1' }, async (request) => {
    try {
        const { teamId, round, player, cleanStats, finalPoints, actionBy } = request.data || {};
        if (!teamId || !player) {
            throw new HttpsError('invalid-argument', 'Missing teamId or player object');
        }
        ...
        if (!teamSnap.exists) {
            throw new HttpsError('not-found', `Team doc not found for teamId: ${teamId}`);
        }
        ...
        throw new HttpsError('internal', err.message || 'Failed to update player points');
    }
});