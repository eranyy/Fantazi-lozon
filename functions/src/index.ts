    ...
    try {
        ...
        if (isAiQuery && !isControlCmd) {
            ...
            if (isRealTeamQuery) {
                ...
                const supportingManagers = Object.values(fanProfiles).filter((f: any) => f.realFanOf?.includes(teamDiscussed) || teamDiscussed.includes(f.realFanOf));

                let msg = `🗣️ *דיון לוהט על ${teamDiscussed}! (ניתוח אובייקטיבי מבית לוזון AI)* ⚽\n\n`;
                if (supportingManagers.length > 0) {
                    const managersStr = supportingManagers.map((f: any) => `*${f.managers}* (קבוצת *${f.teamName}*)`).join(', ');
