        ...
        if (diffHours >= 1.0 && diffHours <= 2.5) {
            ...
            if (!logDoc.exists) {
                ...
                const pushPromises = usersSnap.docs.map(async (uDoc) => {
                    ...
                    if (startingCount < 11) {
                        return sendPushNotificationHelper(`⚠️ הרכב חסר לפני שריקת הפתיחה!`, `שים לב! חסר לך שחקן בהרכב הפותח או שיש לך שחקן שלא משחק השבוע. כנס לעדכן!`, uDoc.id);
                    }
                    else {
                        return sendPushNotificationHelper(`🚨 שעתיים בלבד לסגירת החלון!`, `שעתיים בלבד לסגירת החלון! כנס לנעול את 11 השחקנים שלך לפני שהמשחק הראשון יוצא לדרך.`, uDoc.id);
                    }
                });
