...
async function sendPushNotificationHelper(title, message, targetUserId, excludeUserId) {
    try {
        if (!title || !message)
            return { success: false, reason: 'Missing title/message' };
        const tokens = [];
        if (targetUserId && targetUserId !== 'ALL') {
            const userDoc = await db.collection('users').doc(targetUserId).get();
            if (userDoc.exists) {
...
exports.onUserTransfersUpdated = (0, firestore_1.onDocumentUpdated)({ region: 'us-west1', document: 'users/{userId}' }, async (event) => {
    try {
        const beforeData = event.data?.before.data();
        const afterData = event.data?.after.data();
        ...
        if (afterTransfers.length <= beforeTransfers.length)
            return;
        const beforeIds = new Set(beforeTransfers.map((t) => t.id || `${t.type}_${t.playerName}_${t.date}`));
        const newTransfers = afterTransfers.filter((t) => !beforeIds.has(t.id || `${t.type}_${t.playerName}_${t.date}`));
    ...
    try {
        ...
        const calculateOptimalLineupServer = (squad) => {
            ...
            const getPos = (pStr) => {
                ...
                if (p.includes('GK') || p.includes('שוער'))
                    return 'GK';
                if (p.includes('DEF') || p.includes('הגנה') || p.includes('בלם') || p.includes('מגן'))
                    return 'DEF';
                if (p.includes('MID') || p.includes('קשר') || p.includes('קישור'))
                    return 'MID';
                if (p.includes('FWD') || p.includes('חלוץ') || p.includes('התקפה'))
                    return 'FWD';
                return 'MID';
            };
            const gks = [];
        ...
        if (isStructuredAiQuery && !isControlCmd) {
            ...
            if (isRealTeamQuery) {
                ...
                const teamMatches = realMatches.filter((m) => ((m.homeTeam || '').includes(teamDiscussed) || (m.awayTeam || '').includes(teamDiscussed)) &&
        ...
        if (clearKeywords.some(kw => cleanP === kw || cleanP.includes(kw))) {
            ...
            for (const doc of recentBotMsgsSnap.docs) {
                ...
                if (data.idMessage) {
                    try {
                        await axios_1.default.post(`${greenHost}/waInstance${greenId}/deleteMessage/${greenToken}`, {
                            chatId: groupChatId,
...
exports.broadcastRoundCloseToWhatsApp = (0, https_1.onCall)({ region: 'us-west1' }, async (request) => {
    try {
        ...
        try {
            ...
            await sendPushNotificationHelper(`🎙️ מחזור ${round} נסגר רשמית!`, `מחזור ${round} נסגר רשמית! פוסט הסיכום של האנליסט AI והטבלה המעודכנת עלו עכשיו לאפליקציה.`, 'ALL');
            // B. Personal Match Result Push Notifications
            const fixturesSnap = await db.doc('leagueData/fixtures').get();
            if (fixturesSnap.exists) {
                const roundsData = fixturesSnap.data()?.rounds || [];
                const currentRoundObj = roundsData.find((r) => r.round === round);
            ...
            if (topSnap.exists) {
                ...
                if (topPlayers.length > 0) {
                    ...
                    await sendPushNotificationHelper(`💡 טיפ מהאנליסט`, `טיפ מהאנליסט: ${bestPlayer.name} בכושר שיא עם ${bestPlayer.points} נקודות ב-3 המחזורים האחרונים. שווה לשקול אותו להרכב?`, 'ALL');
                }
            }
        }
        catch (pushErr) {
...
const runRoundReminderLogic = async (force = false) => {
    ...
    const realLines = matchesWithTime.slice(0, 5).map((m) => `• *${m.homeTeam}* 🆚 *${m.awayTeam}* (${m.date} בשעה ${m.time}${m.tvChannel ? ` | ${m.tvChannel}` : ''})`).join('\n');
...
const runSheetSyncLogic = async () => {
    ...
    const mergedMatches = matches.map(m => {
        const existing = existingMatches.find((em) => em.round === m.round &&
...
exports.checkMatchDeadlinesAndNotify = (0, scheduler_1.onSchedule)({ region: 'us-west1', schedule: 'every 1 hours' }, async (event) => {
    try {
        ...
        if (!realSnap.exists)
            return;
        const matches = realSnap.data()?.matches || [];
        const upcomingMatches = matches.filter((m) => m.round === currentRound && m.date);
        ...
        if (diffHours >= 23 && diffHours <= 25) {
            ...
            if (!logDoc.exists) {
                ...
                await sendPushNotificationHelper(`⏳ 24 שעות לסגירת המחזור!`, `נשארו 24 שעות לנעילת ההרכבים למחזור ${currentRound}! בדקת שאין לך שחקנים פצועים או מורחקים בהרכב?`, 'ALL');
...
exports.updateLivePlayerPoints = (0, https_1.onCall)({ region: 'us-west1' }, async (request) => {
    try {
        ...
        let updatedLineup = (0, playerUtils_1.updatePlayerInList)(freshTeam.published_lineup, player, finalPoints, cleanStats);
        let updatedSubsOut = (0, playerUtils_1.updatePlayerInList)(freshTeam.published_subs_out, player, finalPoints, cleanStats);
        let updatedSquad = (0, playerUtils_1.updatePlayerInList)(freshTeam.squad, player, finalPoints, cleanStats);
        const foundInLineup = updatedLineup.some((p) => (0, playerUtils_1.isSamePlayer)(p, player));
        const foundInSubsOut = updatedSubsOut.some((p) => (0, playerUtils_1.isSamePlayer)(p, player));
        ...
        let updatedRLineup = (0, playerUtils_1.updatePlayerInList)(currentRData.lineup || freshTeam.published_lineup || [], player, finalPoints, cleanStats);
        let updatedRSubsOut = (0, playerUtils_1.updatePlayerInList)(currentRData.subsOut || freshTeam.published_subs_out || [], player, finalPoints, cleanStats);
        const rFoundInLineup = updatedRLineup.some((p) => (0, playerUtils_1.isSamePlayer)(p, player));
        const rFoundInSubsOut = updatedRSubsOut.some((p) => (0, playerUtils_1.isSamePlayer)(p, player));
